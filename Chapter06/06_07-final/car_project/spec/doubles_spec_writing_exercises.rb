describe 'Doubles' do
  ...
  it "allows stubbing methods" do
    ...
    test = double('talking')
    allow(test).to receive(:shout)
    expect(test).to respond_to(:shout)
  end
  ...
  it "allows stubbing a response with a block" do
    ...
    dbl2 = double('Shout')
    allow(dbl2).to receive(:hello) { "Echo"}
    expect(dbl2.hello).to eq('Echo')
  end
  ...
  it "allows stubbing responses with #and_return" do
    ...
    dbl2 = double('Shout')
    allow(dbl2).to receive(:hey).and_return("Echo")
    expect(dbl2.hey).to eq('Echo')
  end
  ...
  it "allows stubbing multiple methods with hash syntax" do
    ...
    dbl2 = double("Man")
    allow(dbl2).to receive_messages(:name => 'Sam', :hair_color => 'Dark Brown')
    expect(dbl2.name).to eq('Sam')
    expect(dbl2.hair_color).to eq('Dark Brown')
  end
  ...
  it "allows stubbing with a hash argument to #double" do
    ...
    dbl2 = double('Dog', :name => "Kuma", :breed => "Yorkie")
    expect(dbl2.name).to eq("Kuma")
    expect(dbl2.breed).to eq('Yorkie')
  end
  ...
  it "allows stubbing multiple responses with #and_return" do
    ...
    car = double('Car')
    allow(car).to receive(:drive).and_return('zoom', 'turn', 'brake')
    expect(car.drive).to eq('zoom')
    expect(car.drive).to eq('turn')
    expect(car.drive).to eq('brake')
  end

  context 'with partial test doubles' do

    xit "allows stubbing instance methods on Ruby classes" do
    ...
    it "allows stubbing instance methods on custom classes" do
      ...
      pet = Dog.new
      pet.breed = 'Yorkie'
      expect(pet.breed).to eq('Yorkie')
      allow(pet).to receive(:breed).and_return('Great Dane')
